...
delete_file_if_exists <- function(file) {
  if (fs::file_exists(file)) {
    fs::file_delete(file)
  }
}
delete_dir_if_exists <- function(dir) {
...
render_file <- function(
  ...
  if (fs::file_exists(index_file_name)) {
    stop(sprintf(
      "Stopping to avoid overwriting file.\nFile already exists: %s", index_file_name
    ))
  }
  ...
  on.exit({
    delete_dir_if_exists(index_files_dir)
    delete_file_if_exists(knit_file)
    delete_file_if_exists(readme_file)
    delete_file_if_exists(html_file)
    delete_file_if_exists(index_file_name)
    # For julia/python
    delete_file_if_exists(here::here(file_dir, "index.ipynb"))
  })
  ...
  system(sprintf(
    "%s render '%s' --to html -M keep-md:true --execute-dir '%s'",
    quarto_path, index_file_name, here::here()
  ))
  # system(paste0(
  #   quarto_path,
  ...
  # Render `index.html.md` to github flavored markdown
  ...
  if (fs::dir_exists(md_file_folder)) {
    ...
    delete_dir_if_exists(out_dir_files)
